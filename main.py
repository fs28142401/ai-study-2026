...
subject = st.sidebar.selectbox("Chọn môn học", subjects, format_func=format_subject_name)
...
if mode == "👨 Bố (Admin)":
    ...
    with tab3:
        ...
        if available_lesson_days:
            ...
            st.warning("Chưa có bài học nào.")
    ...
    if students.empty:
        st.warning("Chưa có bé nào được thêm.")
        st.stop()
    ...
    progress_df = pd.read_sql_query("""
        SELECT day_key as "Ngày", status as "Trạng thái", 
               assigned_date as "Giao ngày", completed_date as "Hoàn thành"
    ...
    if not progress_df.empty:
        ...
        submission = st.text_area("Dán code Python hoặc câu trả lời", height=250)
        notebook_link = st.text_input("🔗 Link NotebookLM (nếu có)")
